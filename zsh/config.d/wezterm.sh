...
__bp_precmd_invoke_cmd() {
    ...
    # shellcheck disable=SC2034  # BP_PIPESTATUS is used elsewhere
...
__bp_install() {
    ...
    local prior_trap
    # shellcheck disable=SC2001  # could use ${var/pat/sub} but don't want to debug
...
__wezterm_semantic_precmd() {
  ...
  if [[ "$__wezterm_semantic_precmd_executing" != "0" ]] ; then
    ...
    if [[ -n "$ZSH_NAME" ]] ; then
      # shellcheck disable=SC2025 # this is for zsh
