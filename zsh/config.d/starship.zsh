...
prompt_starship_precmd() {
    ...
    # # use one-line PROMPT for compatibility with some completion functions
    # if [[ -z $STARSHIP_RESET_PROMPT ]]; then
    #     local prompt="$(/opt/homebrew/bin/starship prompt --terminal-width="$COLUMNS" --keymap="${KEYMAP:-}" --status="$STARSHIP_CMD_STATUS" --pipestatus="${STARSHIP_PIPE_STATUS[*]}" --cmd-duration="${STARSHIP_DURATION:-}" --jobs="$STARSHIP_JOBS_COUNT")"
    #     STARSHIP_PROMPT="${prompt##*$'\n'}"
    #     print -rP "${prompt%$'\n'*}"
    # else
    #     unset STARSHIP_RESET_PROMPT
    # fi
}
...
starship_zle-keymap-select() {
    # if [[ $STARSHIP_RESET_PROMPT_NEWLINES == 2 ]]; then
    #     print $'\n'
    # elif [[ $STARSHIP_RESET_PROMPT_NEWLINES == 1 ]]; then
    #     print
    # fi
    # unset STARSHIP_RESET_PROMPT_NEWLINES
    zle reset-prompt
}
...
# only last line of prompt:
# PROMPT='$(/opt/homebrew/bin/starship prompt --terminal-width="$COLUMNS" --keymap="${KEYMAP:-}" --status="$STARSHIP_CMD_STATUS" --pipestatus="${STARSHIP_PIPE_STATUS[*]}" --cmd-duration="${STARSHIP_DURATION:-}" --jobs="$STARSHIP_JOBS_COUNT" | tail -n 1)'
